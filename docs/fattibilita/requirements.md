...
- Analisi del problema: identificazione del problema o dell'opportunità che il progetto intende affrontare.
- Definizione delle funzionalità: prestazioni e caratteristiche che il sistema deve avere per soddisfare le esigenze degli utenti.
- SRS (Software Requirements Specification): documento che descrive in dettaglio i requisiti funzionali e non funzionali del sistema, inclusi i vincoli e le interfacce.
- Convalida delle specifiche: prima di procedere alla di realizzazione è necessario verificare con il commitente (o utente finale) che le specifiche siano corrette e complete, e che soddisfino le esigenze del progetto.
